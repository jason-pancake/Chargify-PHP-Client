...
class ChargifyConnector
{
	...
	private $test_api_key = 'YOUR API KEY HERE';
	private $domain = 'YOUR DOMAIN HERE'; //your chargify domain, e.g. if you have [your-domain].chargify.com, then enter "your-domain" only.
	private $test_domain = 'YOUR DOMAIN HERE';
	...
	public function requestUpdateSubscriptionProratedPreview($subscription_id, $migrationRequest, $format = 'XML') {
		$extension = strtoupper($format) == 'XML' ? '.xml' : '.json';
		$base_url = "/subscriptions/{$subscription_id}/migrations/preview" . $extension;
	...
	public function getUpdateSubscriptionProductProratedPreview($subscription_id, $chargify_product, $include_trial = false, $include_initial_charge = false) {
		...
		$xml = $this->requestUpdateSubscriptionProratedPreview($subscription_id, $chargify_migration->getXML());
		$migration_preview = new SimpleXMLElement($xml);
		return new ChargifyMigrationPreview($migration_preview, $this->test_mode);
	}
